...
// JavaScript to handle the purpose of visit dropdown
document.addEventListener('DOMContentLoaded', function() {
    // Get the purpose of visit dropdown element
    const purposeDropdown = document.getElementById('purpose');

    // Add event listener to the dropdown to update its style when an option is selected
    purposeDropdown.addEventListener('change', function() {
        if (this.value !== '') {
            this.style.color = '#333'; // Change text color to black
        } else {
            this.style.color = '#999'; // Change text color to gray
        }
    });
});
